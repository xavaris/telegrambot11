...
def build_offer_caption(offer: Offer, settings: Settings) -> str:
    ...
    if offer.market_baseline > 0:
        ...
        parts.append(f"<b>Score okazji:</b> {offer.score:.1%} <i>(brak mediany w bazie)</i>")
    ...
    if clean_description:
        parts.extend(["", f"<b>Opis:</b> {html.escape(clean_description[:350])}"])
...
def build_offer_keyboard(offer: Offer) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="🔗 Otwórz ogłoszenie", url=offer.url)]]
    )

...
def _normalize_model_text(text: str) -> str:
    ...
    value = value.replace("iphone se2", "iphone se 2020")
    value = value.replace("iphone se 2", "iphone se 2020")
    value = value.replace("iphone se3", "iphone se 2022")
    value = value.replace("iphone se 3", "iphone se 2022")
    return value


def parse_model(text: str) -> str:
    value = _normalize_model_text(text)
    ...
    pattern = re.compile(
        r"\biphone\s*(11|12|13|14|15|16)\s*(pro\s*max|pro|max|plus|mini)?\b",
        re.IGNORECASE,
    )
    match = pattern.search(value)
    if match:
        number = match.group(1)
        variant = (match.group(2) or "").strip().lower().replace("  ", " ")
        variant = variant.replace("max", "pro max") if variant == "max" else variant
        if variant:
...
def parse_storage(text: str) -> str:
    ...
    match = re.search(r"\b(64|128|256|512)\s*gb\b", clean_text(text), re.IGNORECASE)
    if match:

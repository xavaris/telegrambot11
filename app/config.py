...
class Settings(BaseSettings):
    ...
    MAX_OFFERS_PER_SOURCE: int = 25
    REQUEST_DELAY_MS: int = 800
    DETAILS_PER_SOURCE: int = 12
    CONCURRENT_DETAIL_TASKS: int = 5
    USER_AGENT: str = (
    ...
    ONLY_MODELS: str = ""
    EXCLUDED_KEYWORDS: str = "icloud,blokada,na części,uszkodzony"
    PREFERRED_LOCATIONS: str = ""
    PREFERRED_REGIONS: str = ""

    MIN_DEAL_SCORE: float = 0.08
    MIN_PRICE: float = 500
    MAX_PRICE: float = 10000

    MAX_PRICE_BY_MODEL_JSON: str = "{}"
    REFERENCE_PRICES_JSON: str = "{}"
    ...
    def reference_prices(self) -> Dict[str, float]:
        try:
            data = json.loads(self.REFERENCE_PRICES_JSON or "{}")
            return {str(k).lower(): float(v) for k, v in data.items()}
        except Exception:
            return {}

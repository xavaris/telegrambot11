...
class Database:
    ...
    async def init(self) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """)
            await db.execute("CREATE INDEX IF NOT EXISTS idx_seen_offers_unique_key ON seen_offers (unique_key)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_seen_offers_created_at ON seen_offers (created_at)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_market_baselines_model_storage ON market_baselines (model, storage)")
            await db.commit()
        logger.info("Baza danych gotowa: %s", self.db_path)

    async def has_seen(self, offer: Offer) -> bool:
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute("SELECT 1 FROM seen_offers WHERE unique_key = ? LIMIT 1", (offer.unique_key,))
    ...
    async def mark_seen(self, offer: Offer) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            ...
            """, (
                offer.unique_key, offer.source, offer.url, offer.title, offer.price,
                offer.model, offer.storage, offer.condition,
            ))
            await db.commit()
    ...
    async def upsert_market_baseline(self, model: str, storage: str, baseline_price: float, sample_size: int, scope: str) -> None:
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute("""
            ...
            """, (
                ...
                baseline_price, sample_size, scope,
            ))
            await db.commit()

    async def get_market_baseline(self, model: str, storage: str) -> tuple[float, int, str] | None:

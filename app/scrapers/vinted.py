...
class VintedScraper(BaseScraper):
    ...
    async def _extract_image(self, page) -> str:
        # Najpierw spróbuj meta og:image
        try:
        ...
        try:
            ...
            for i in range(min(img_count, 12)):
                ...
                # Odrzucamy ewidentne ikony / avatary / loga
                bad_parts = [
                    "avatar",
                    "icon",
                    "logo",
    ...
    async def _extract_description(self, page) -> str:
        ...
        for selector in selectors:
            try:
                ...
                # Jeśli to wygląda jak śmietnik z całej strony, pomijamy
                if any(snippet in lower_value for snippet in bad_snippets):
                    continue

                # Przycinamy do sensownej długości
                if len(value) > 500:
                    value = value[:500].strip()

                # Zbyt krótki opis pomijamy
                if len(value) < 10:
                    continue

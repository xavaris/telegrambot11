...
class MarketBaselineService:
    ...
    async def _refresh_model_only(self, browser, model: str) -> None:
        ...
        prices = self._extract_prices_for_exact_match(
            offers=offers,
            model=model,
            storage="",
        )
        ...
        if len(prices) < self.settings.BASELINE_MIN_SAMPLES_FOR_MODEL:
            logger.info(
                "[baseline] Za mało danych dla modelu=%s | samples=%s",
                model,
                len(prices),
            )
            return
        ...
        logger.info(
            "[baseline] model=%s | baseline=%s | samples=%s",
            model,
            baseline,
            len(prices),
        )
    ...
    def _extract_prices_for_exact_match(
        self,
        offers: list[Offer],
        model: str,
        storage: str,
        ...
        for offer in offers:
            ...
            if storage_norm:
                if (offer.storage or "").upper().strip() != storage_norm:
                    continue
    ...
    def _build_allegro_url(self, query: str) -> str:
        slug = quote(query)
        return f"https://allegrolokalnie.pl/oferty/q/{slug}"
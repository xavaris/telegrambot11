...
class FlipperService:
    ...
    async def process_offer(self, offer: Offer) -> None:
        ...
        try:
            ...
            logger.info("FILTERED | source=%s | model=%s | storage=%s | price=%s | score=%s | baseline=%s | title=%s",
    ...
    async def publish_offer(self, offer: Offer) -> None:
        ...
        logger.info("Publikuję ofertę: %s | %s | %s | %.2f", offer.source, offer.model, offer.storage, offer.price)
